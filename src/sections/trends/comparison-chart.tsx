...
export default function ComparisonChart({ stations, pollutant, pollutantLabel, pollutantUnit }: TrendsChartProps) {
  // two sensor IDs for comparison
  const [sensorId1, setSensorId1] = useState<string | undefined>(stations[0]?.sensorId);
  const [sensorId2, setSensorId2] = useState<string | undefined>(stations[1]?.sensorId);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const res1 = await getAnalyticsTimeSeries(sensorId1 ?? '', start, end);
        const res2 = await getAnalyticsTimeSeries(sensorId2 ?? '', start, end);
        setData1(
          Array.isArray(res1)
            ? res1.map((item) => ({
                ...item,
  ...
  const pollutantSeries = (data: HistoricalData[]) =>
    data.map((item) => ({
      ...
      y: selectedPollutant === 'aqi' ? item.aqi : selectedPollutant === 'pm25' ? item.pm25 : item.pm10
    }));
      ...
      <CardContent>
        <Box display="flex" flexWrap="wrap" gap={2} mb={3}>
          ...
          <FormControl sx={{ minWidth: 180 }}>
            <Select value={sensorId1 ?? ''} onChange={(e) => setSensorId1(e.target.value)} displayEmpty>
              {stations.map((s) => (
          ...
          {/* Sensor 2 */}
          <FormControl sx={{ minWidth: 180 }}>
            <Select value={sensorId2 ?? ''} onChange={(e) => setSensorId2(e.target.value)} displayEmpty>
              {stations.map((s) => (
                <MenuItem key={s.sensorId} value={s.sensorId}>
                  {s.name}
                </MenuItem>
